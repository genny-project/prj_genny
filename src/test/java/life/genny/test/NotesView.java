...
public class NotesView extends GennyJbpmBaseTest {
	...
	public Frame3 getNotes() {
		...
		/* get the theme */
		Theme THM_SIDEBAR = VertxUtils.getObject(serviceToken.getRealm(), "", "THM_SIDEBAR", Theme.class,
				serviceToken.getToken());
		Theme THM_SIDEBAR_WIDTH = VertxUtils.getObject(serviceToken.getRealm(), "", "THM_SIDEBAR_WIDTH", Theme.class,
				serviceToken.getToken());
		Theme THM_SCROLL_VERTICAL = VertxUtils.getObject(serviceToken.getRealm(), "", "THM_SCROLL_VERTICAL", Theme.class,
				serviceToken.getToken());

		Frame3 FRM_TAB_HEADER_NOTES = getTabHeaderNotes();
		Frame3 FRM_TAB_CONTENT_NOTES = getTabContentNotes();

		Frame3 FRM_NOTES = Frame3.builder("FRM_NOTES")
											.addTheme(THM_SIDEBAR).end()
											.addTheme(THM_SIDEBAR_WIDTH).end()
											.addTheme(THM_SCROLL_VERTICAL, ThemePosition.WRAPPER).end()
											.addFrame(FRM_TAB_HEADER_NOTES, FramePosition.NORTH).end()
											.addFrame(FRM_TAB_CONTENT_NOTES, FramePosition.CENTRE).end()
											.build();
	...
	public Frame3 getTabHeaderNotes() {
		...
		Frame3 FRM_TAB_ITEM_NOTES = Frame3.builder("FRM_TAB_ITEM_NOTES").addTheme(THM_TAB).end()
				.addTheme(THM_PROJECT_COLOR_PRIMARY_VARIANT_LIGHT).end().addTheme(THM_TAB_WIDTH, ThemePosition.WRAPPER).end()
				.question("QUE_TAB_NOTES_VIEW").end().build();

		Frame3 FRM_TAB_ITEM_NOTIFICATION = Frame3.builder("FRM_TAB_ITEM_NOTIFICATION").addTheme(THM_TAB).end()
				.addTheme(THM_PROJECT_COLOR_PRIMARY_VARIANT_LIGHT).end().addTheme(THM_TAB_WIDTH, ThemePosition.WRAPPER).end()
				.question("QUE_TAB_NOTIFICATIONS_VIEW").end().build();

		Frame3 FRM_TAB_HEADER_NOTES = Frame3.builder("FRM_TAB_HEADER_NOTES").addTheme(THM_TAB_HEADER, ThemePosition.FRAME)
				.end().addTheme(THM_PROJECT_COLOR_PRIMARY_VARIANT_LIGHT, ThemePosition.FRAME).end()
				.addTheme(THM_FRAME_ALIGN_WEST, ThemePosition.WEST).end().addFrame(FRM_TAB_ITEM_NOTES, FramePosition.WEST).end()
				.addFrame(FRM_TAB_ITEM_NOTIFICATION, FramePosition.WEST).end().build();

		return FRM_TAB_HEADER_NOTES;
	}
	...
	public Frame3 getTabContentNotes() {
		...
		/* get the theme */
		Theme THM_PROJECT_COLOR_SURFACE = VertxUtils.getObject(serviceToken.getRealm(), "", "THM_PROJECT_COLOR_SURFACE", Theme.class,
				serviceToken.getToken());
		Theme THM_PROJECT_COLOR_BACKGROUND = VertxUtils.getObject(serviceToken.getRealm(), "", "THM_PROJECT_COLOR_BACKGROUND", Theme.class,
				serviceToken.getToken());
				

		// Frame3 FRM_TAB_CONTENT_NOTES = Frame3.builder("FRM_TAB_CONTENT_NOTES")
		// 								.addTheme(THM_PROJECT_COLOR_SURFACE).end()
		// 								.addTheme(THM_PROJECT_COLOR_BACKGROUND).end()
		// 								.build();

		Frame3 FRM_TAB_CONTENT_NOTES = VertxUtils.getObject(serviceToken.getRealm(), "", "FRM_TAB_CONTENT_NOTES", Frame3.class,
				serviceToken.getToken());
		
		return FRM_TAB_CONTENT_NOTES;
	}
	...
	public void sendOuterFrame() {
		...
		Frame3 FRM_APP_CONTENT = getOuterFrames();

		Frame3 frame = Frame3.builder("FRM_APP").addFrame(FRM_APP_CONTENT, FramePosition.CENTRE).end().build();

		QDataBaseEntityMessage msg = FrameUtils2.toMessage(frame, serviceToken, askSet, contextListMap, virtualAskMap);
		...
		System.out.print("Sending bucket");
		testProcessView();
	}
	...
	public Frame3 getOuterFrames() {
		...
		Frame3 FRM_SIDEBAR = VertxUtils.getObject(serviceToken.getRealm(), "", "FRM_SIDEBAR", Frame3.class,
				serviceToken.getToken());
		Frame3 FRM_CONTENT_WRAPPER = VertxUtils.getObject(serviceToken.getRealm(), "", "FRM_CONTENT_WRAPPER", Frame3.class,
				serviceToken.getToken());
		...
		Frame3 FRM_APP_CONTENT = Frame3.builder("FRM_APP_CONTENT").addTheme(THM_PROJECT, ThemePosition.FRAME).end()
				.addTheme(THM_FONT_FAMILY_ROBOTO, ThemePosition.FRAME).end().addFrame(FRM_HEADER, FramePosition.NORTH).end()
				.addFrame(FRM_SIDEBAR, FramePosition.WEST).end().addFrame(FRM_NOTES, FramePosition.EAST).end()
				.addFrame(FRM_CONTENT_WRAPPER, FramePosition.CENTRE).end().addFrame(FRM_FOOTER, FramePosition.SOUTH).end()
				.build();

		return FRM_APP_CONTENT;
	...
	public void sendCards() {
		...
		Theme THM_SCROLL_VERTICAL2 = Theme.builder("THM_SCROLL_VERTICAL2").addAttribute().overflowY("auto").end()
				.addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();

		Theme THM_PADDING_15 = Theme.builder("THM_PADDING_15").addAttribute().padding(15).end()
				.addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();
	...
	public void testProcessView() {
		...
		Theme THM_BUCKET_HEADER_GROUP_WRAPPER = Theme.builder("THM_BUCKET_HEADER_GROUP_WRAPPER").addAttribute()
				.flexDirection("row").width("100%").paddingLeft(8).paddingRight(4).end().build();

		Theme THM_BUCKET_HEADER_GROUP_HEADER_WRAPPER = Theme.builder("THM_BUCKET_HEADER_GROUP_HEADER_WRAPPER")
				.addAttribute().flexGrow(1).marginBottom(4).end().build();

		Theme THM_BUCKET_HEADER_GROUP_LABEL = Theme.builder("THM_BUCKET_HEADER_GROUP_LABEL").addAttribute()
				.justifyContent("center").marginBottom(0).end().build();
		Theme THM_FONT_FAMILY_ROBOTO = Theme.builder("THM_FONT_FAMILY_ROBOTO").addAttribute().fontFamily("Roboto").end()
				.build();
		...
		/*
		 * bucketHeaderContext.add(new Context(ContextType.THEME,
		 * bucketUtils.getThemeBe(THM_BH_GROUP_WRAPPER),
		...
		bucketHeaderContext.add(new Context(ContextType.THEME, bucketUtils.getThemeBe(THM_BUCKET_HEADER_GROUP_WRAPPER),
				VisualControlType.GROUP_WRAPPER, 1.0));
		bucketHeaderContext.add(new Context(ContextType.THEME,
				bucketUtils.getThemeBe(THM_BUCKET_HEADER_GROUP_HEADER_WRAPPER), VisualControlType.GROUP_HEADER_WRAPPER, 1.0));
		...
		try {
			...
			if (searchBeList != null && !searchBeList.isEmpty()) {
				for (SearchEntity searchBe : searchBeList) {
					...
					// bucket.getFrames().add(new FrameTuple3(bucketFooter, FramePosition.SOUTH,
					// 1.0));
					...
					// virtualAskMap.put(bucketFooterAsk.getQuestionCode(), new
					// QDataAskMessage(bucketFooterAsk));
			...
			Frame3 FRM_NOTES_VIEW = Frame3.builder("FRM_NOTES_VIEW").addFrame(FRM_BUCKET_WRAPPER, FramePosition.CENTRE)
					.end().build();
		...
		//	Frame3 FRM_TAB_CONTENT_NOTES = getTabContentNotes();
			...
			Frame3 FRM_TAB_CONTENT_NOTES = Frame3.builder("FRM_TAB_CONTENT_NOTES")
								.addTheme(THM_PROJECT_COLOR_SURFACE).end()
								.addTheme(THM_PROJECT_COLOR_BACKGROUND).end()
								.addFrame(FRM_NOTES_VIEW, FramePosition.NORTH).end()
								.build();
			//FRM_TAB_CONTENT_NOTES.getFrames().add(new FrameTuple3(FRM_NOTES_VIEW, FramePosition.NORTH, 1.0));

			QDataBaseEntityMessage msg = FrameUtils2.toMessage(FRM_TAB_CONTENT_NOTES, serviceToken, askSet, contextListMap,
					virtualAskMap);
			msg.setToken(userToken.getToken());
			VertxUtils.writeMsg("webcmds", JsonUtils.toJson(msg));
	...
	public Frame3 getBucketFrame(String name, String target, String questionCode) {
		...
		Theme THM_BACKGROUND_E4E4E4 = Theme.builder("THM_BACKGROUND_E4E4E4").addAttribute().backgroundColor("#E4E4E4").end()
				.build();

		Theme THM_BUCKET_COLUMN = Theme.builder("THM_BUCKET_COLUMN").addAttribute().minWidth(344).width("100%")
				.textAlign("center").flexDirection("column").end().addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false)
				.end().build();

		Theme THM_BUCKET_COLUMN_PADDING2 = Theme.builder("THM_BUCKET_COLUMN_PADDING2").addAttribute()
				// .padding(20)
				.justifyContent("flex-start").end().addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();

		Theme THM_SCROLL_VERTICAL = Theme.builder("THM_SCROLL_VERTICAL").addAttribute().overflowY("auto").end()
				.addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();

		Theme THM_BUCKET_PANEL_NORTH = Theme.builder("THM_BUCKET_PANEL_NORTH").addAttribute().paddingX(4).paddingTop(8)
				.end().addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();
		Theme THM_BUCKET_PANEL_CENTRE = Theme.builder("THM_BUCKET_PANEL_CENTRE").addAttribute().padding(4).end()
				.addAttribute(ThemeAttributeType.PRI_IS_INHERITABLE, false).end().build();

		/* build the frame */
		Frame3 bucket = Frame3.builder(name).addTheme(THM_BACKGROUND_NONE).end()
				.addTheme(THM_BACKGROUND_E4E4E4, ThemePosition.WRAPPER).end().addTheme(THM_BUCKET_COLUMN, ThemePosition.WRAPPER)
				.end().addTheme(THM_BUCKET_COLUMN_PADDING2, ThemePosition.CENTRE).end()
				.addTheme(THM_SCROLL_VERTICAL, ThemePosition.CENTRE).end().addTheme(THM_BUCKET_PANEL_NORTH, ThemePosition.NORTH)
				.end().addTheme(THM_BUCKET_PANEL_CENTRE, ThemePosition.CENTRE).end().build();
	...
	public Ask getBucketHeaderAsk(Map<String, ContextList> contextListMap, GennyToken serviceToken) {
		...
		// NEW THEMES START HERE
		// THM_BUCKET_HEADER_GROUP_WRAPPER =
		// Theme.builder("THM_BUCKET_HEADER_GROUP_WRAPPER")
		// .addAttribute()
		// .flexDirection("row")
		// .end()
		// .build();
		...
		// row1Context.add(new Context(ContextType.THEME,
		// bucketUtils.getThemeBe(THM_DISPLAY_HORIZONTAL),
		// VisualControlType.GROUP_WRAPPER, 1.0));
		// row1Context.add(new Context(ContextType.THEME,
		// bucketUtils.getThemeBe(THM_BH_ROW_ONE_GRP_WRAPPER),
		// VisualControlType.GROUP_WRAPPER, 1.0));
		// row1Context.add(new Context(ContextType.THEME,
		// bucketUtils.getThemeBe(THM_BH_ROW_ONE_GRP_LABEL),
		// VisualControlType.GROUP_LABEL, 1.0));
		// // row1Context.add(new Context(ContextType.THEME,
		// bucketUtils.getThemeBe(THM_BH_ROW_ONE_GRP_CONTENT_WRAPPER),
		// // VisualControlType.GROUP_CONTENT_WRAPPER, 1.0));
		...
		bucketCountContextList.add(
				new Context(ContextType.THEME, bucketUtils.getThemeBe(THM_QUESTION_GRP_LABEL), VisualControlType.GROUP, 1.0));
		...
		// contextListMap.put("QUE_BUCKET_HEADER_ROW_TWO_GRP", new
		// ContextList(row2Context));
	...
	public Ask getBucketFooterAsk(Map<String, ContextList> contextListMap, GennyToken serviceToken) {
		...
		nextBucketContext.add(new Context(ContextType.THEME, bucketUtils.getThemeBe(THM_ICON), VisualControlType.VCL, 1.0));
		...
		prevBucketContext.add(new Context(ContextType.THEME, bucketUtils.getThemeBe(THM_ICON), VisualControlType.VCL, 1.0));
